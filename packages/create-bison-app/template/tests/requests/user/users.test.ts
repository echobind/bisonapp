...
describe('users query', () => {
  describe('not logged in', () => {
    it('returns an Unauthorized error', async () => {
      await expect(trpcRequest().user.users()).rejects.toThrowErrorMatchingInlineSnapshot(
        `"UNAUTHORIZED"`
      );
    });
  });
  ...
  describe('logged in as a user', () => {
    it('returns an Unauthorized error', async () => {
      ...
      await expect(trpcRequest(user).user.users()).rejects.toThrowErrorMatchingInlineSnapshot(
        `"UNAUTHORIZED"`
      );
    });
  });
  ...
  describe('logged in as an admin', () => {
    it('returns the user objects', async () => {
      ...
      const users = await trpcRequest(user1).user.users();
  ...
  describe('trying to view another users email', () => {
    it('returns undefined for the email', async () => {
      ...
      const response = await trpcRequest(user1).user.user({ id: user2.id });
